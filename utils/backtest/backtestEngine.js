...
export class BacktestEngine {
  ...
  handleActiveOrder(order, candle) {
    ...
    // Use the LimitOrderHandler to process the order and update its edge data
    const result = this.limitOrderHandler.processOrder(order, candle);
    
    if (result.filled) {
      // Add calculated exit levels to the trade
      const { takeProfit, stopLoss } = this.calculateExitLevels(result.trade, candle);
      result.trade.takeProfit = takeProfit;
      result.trade.stopLoss = stopLoss;
      
      return result;
    }
    
    // Update the order's edge data using historical data
    if (!result.cancelled && !result.filled) {
      order.edges = result.order.edges; // Update with fresh edge data
    }
  ...
  handlePivotSignal(pivot, candle) {
    ...
    if (isBuySetup || isSellSetup) {
      ...
      if (avgMove > 0) {
        ...
        // Use our specialized LimitOrderHandler to create the order
        // This will handle edge data inheritance from pivot with proper fallbacks
