...
function checkWindowBasedCascade(primaryPivot, detector, oneMinuteCandles, multiPivotConfig) {
    ...
    for (const tf of confirmingTimeframes) {
        ...
        const targetSignal = tf.opposite ? 
...
async function runMultiTimeframeBacktest() {
    ...
    // Delayed entry helper based on config
    const entryDelayMs = (tradeConfig.entryDelayMinutes || 0) * 60 * 1000;
    const getDelayedExecution = (baseTime, fallbackPrice) => {
        const targetTime = baseTime + entryDelayMs;
        // Prefer the first candle at or after the target time
        const candleAfter = oneMinuteCandles.find(c => c.time >= targetTime);
        if (candleAfter) {
            return { time: candleAfter.time, price: candleAfter.close };
        }
        // Fallback: if no future candle (end of data), use last candle available
        const last = oneMinuteCandles[oneMinuteCandles.length - 1];
        if (last) {
            return { time: last.time, price: last.close };
        }
        // Final fallback to provided price/time
        return { time: targetTime, price: fallbackPrice };
    };
    ...
    for (const primaryPivot of primaryPivots) {
        ...
        if (cascadeResult) {
            ...
            if (shouldOpenTrade && capital > 0) {
                ...
                // Decide flipping/opening logic based on threshold
                if (hasOppositeOpen && tradeConfig.switchOnOppositeSignal) {
                    if (oppositeSignalCounts[tradeType] >= flipThreshold) {
                        // Perform flip: close opposite trades at delayed time, then open new trade
                        const delayedFlip = getDelayedExecution(cascadeResult.executionTime, cascadeResult.executionPrice);
                        const execTime = delayedFlip.time;
                        const flipExitPriceRaw = delayedFlip.price;
                        ...
                        for (let j = openTrades.length - 1; j >= 0; j--) {
                            ...
                            if (tradeConfig.showTradeDetails) {
                                ...
                                const pnlColor = pnl >= 0 ? colors.green : colors.red;
                                const pnlText = `${pnlColor}${pnl >= 0 ? '+' : ''}${formatNumberWithCommas(pnl)}${colors.reset}`;
                                console.log(`  \x1b[35;1m└─> [FLIP ${oppositeSignalCounts[tradeType]}/${flipThreshold}] ${tradeTypeLabel} trade closed @ ${formatNumberWithCommas(flipExitPriceRaw)}. PnL: ${pnlText}${colors.reset}`);
                            }
                        ...
                        if (tradeConfig.showTradeDetails) {
                            console.log(`  ${colors.yellow}└─> [WAITING] ${tradeType.toUpperCase()} signal ${oppositeSignalCounts[tradeType]}/${flipThreshold} - need ${flipThreshold - oppositeSignalCounts[tradeType]} more opposite signals to flip${colors.reset}`);
                        }
                        continue; // Skip to next primary pivot
                    }
                }
                ...
                if (hasSameDirectionOpen) {
                    ...
                    if (tradeSize > 0) {
                        // Apply delayed execution to the new trade as well
                        const delayed = getDelayedExecution(cascadeResult.executionTime, cascadeResult.executionPrice);
                        const delayedCascade = { ...cascadeResult, executionTime: delayed.time, executionPrice: delayed.price };
                        const trade = createTrade(tradeType, delayedCascade, tradeSize, tradeConfig.takeProfit, tradeConfig.stopLoss);
