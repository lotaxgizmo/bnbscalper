...
async function placeOrder() {
  ...
  const side = 'Buy'; 
  const leverage = 1; // Set your desired leverage
  ...
  const fixedAmount = 600; // Fixed USDT amount (when mode = 'fixed') - increased for minimum order size

  // Get market price
  const entryPrice = await getMarketPrice(symbol);
  
  // Calculate USDT amount based on mode
  let usdtAmount;
  if (amountMode === 'percentage') {
    // For testnet, use mock balance if balance API fails
    try {
      const balance = await getAccountBalance();
      usdtAmount = (balance.availableBalance * usePercentage / 100) * leverage;
      ...
      usdtAmount = (1000 * usePercentage / 100) * leverage;
      console.log('MOCK BALANCE:', 1000, 'USDT');
    }
  } else {
    usdtAmount = fixedAmount * leverage;
